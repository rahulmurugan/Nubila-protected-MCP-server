...
// Determine if we're in production (Railway, Heroku, etc.)
const isProduction = process.env.NODE_ENV === 'production' || 
                   process.env.RAILWAY_ENVIRONMENT || 
                   process.env.HEROKU_APP_NAME;

// Create FastMCP server instance with appropriate transport
...
console.log('Transport:', isProduction ? `HTTP (port ${process.env.PORT || 3000})` : 'STDIO');
console.log('\nToken Requirements:');
...
async function main() {
  try {
    await mcp.start();
    console.log(`\nNubila MCP server (protected) is running on ${isProduction ? 'HTTP' : 'STDIO'}`);
...
if (isProduction) {
  ...
  const healthCheck = () => {
    return {
      ...
      service: 'nubila-protected-mcp-server',
      version: '1.0.0'
    };
  };
  
  // Register health check tool
  mcp.tool('health', 'Health check endpoint', { type: 'object', properties: {} }, healthCheck);
}
